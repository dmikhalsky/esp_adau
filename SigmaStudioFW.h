 ...
 */
    ...
    int32_t SIGMASTUDIOTYPE_FIXPOINT_CONVERT(double value) { return round(value * (0x01 << 24)); }
#else
...
void SIGMA_SET_LEVEL(int level, int levels, int _addr, byte data[][4]) {

    int min_value = (levels  - 1) / 2 - (levels - 1);
    
    Serial.println(min_value);
    int level_idx = level - min_value;

    for (int i = 0; i < 4; i++) {

        if (data[level_idx][i] < 16) {
            Serial.print('0');
        }
        Serial.print(data[level_idx][i], HEX);
        Serial.print(' ');
    }

    SIGMA_WRITE_REGISTER_BLOCK(_addr, 4, data[level_idx]);
